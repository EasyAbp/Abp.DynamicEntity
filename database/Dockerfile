...
WORKDIR /templates/service/host/EasyAbp.Abp.Dynamic.Host
RUN dotnet restore
RUN dotnet ef migrations script -i -o migrations-Dynamic.sql
...
COPY --from=build /templates/service/host/EasyAbp.Abp.Dynamic.Host/migrations-Dynamic.sql migrations-Dynamic.sql
